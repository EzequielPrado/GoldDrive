...
const ClientDashboard = () => {
    ...
    if (pickupLocation && destLocation && step === 'confirm') {
        ...
        service.route({
            ...
            if (status === 'OK' && result) {
                ...
                showError("Caminho não encontrado.");
                setStep('search');
            }
  ...
  const applyCoupon = async () => {
      ...
      try {
          ...
          if (error || !data) throw new Error("Inexistente.");
          setAppliedCoupon(data);
          showSuccess("Cupom ok!");
      } catch (e: any) { showError(e.message); setAppliedCoupon(null); }
  ...
  const calculatePrice = useCallback((catId?: string) => {
      ...
      if (isNight) {
          ...
          // Lógica de Faixas (Se não for noite)
          const dist1 = Number(rules.dist_1 || 0);
          const price1 = Number(rules.price_1 || 0);
          const dist2 = Number(rules.dist_2 || 0);
          const price2 = Number(rules.price_2 || 0);

          let thresholds = [];
          if (price1 > 0 && dist1 > 0) thresholds.push({ dist: dist1, price: price1 });
          if (price2 > 0 && dist2 > 0) thresholds.push({ dist: dist2, price: price2 });
  ...
  const confirmRide = async () => {
    ...
    if (paymentMethod === 'WALLET' && (userProfile?.balance || 0) < price) { 
        setMissingAmount(price - (userProfile?.balance || 0)); setShowBalanceAlert(true); return; 
    }
    ...
    try { 
        const success = await requestRide(
            ...
            price, `${routeDistance.toFixed(1)} km`, category.name, paymentMethod,
            stops.filter(s=>s)
        ); 
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-gray-100 font-sans text-slate-900 relative">
      <div className="absolute inset-0 z-0">
        <GoogleMapComponent pickupLocation={step === 'confirm' || step === 'active' ? pickupLocation : null} destinationLocation={step === 'confirm' || step === 'active' ? destLocation : null} driverLocation={ride?.driver_details?.current_lat ? { lat: ride.driver_details.current_lat, lon: ride.driver_details.current_lng } : null} stops={stops.length > 0 ? stops : null} />
      </div>
      ...
      <div className="absolute top-4 left-4 z-20">
          <Button variant="ghost" size="icon" className="bg-white/80 backdrop-blur-xl h-10 w-10 rounded-xl shadow-lg border border-white/20" onClick={() => navigate('/profile')}><User className="h-5 w-5 text-slate-700" /></Button>
      </div>
      ...
      <div className="absolute inset-0 z-10 flex flex-col justify-end pb-28 pointer-events-none px-4">
              ...
              <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                      <div className="flex gap-2">
                        ...
                        <Button size="icon" variant="outline" className="h-14 w-14 rounded-2xl shrink-0 border-slate-100 bg-slate-50" onClick={getCurrentLocation} disabled={gpsLoading}>{gpsLoading ? <Loader2 className="animate-spin" /> : <MousePointer2 className="w-5 h-5 text-slate-400" />}</Button>
                      </div>
                      {stops.map((stop, index) => (
                          <div key={index} className="flex gap-2 animate-in slide-in-from-left">
                              ...
                              <Button size="icon" variant="outline" className="h-14 w-14 rounded-2xl text-red-400" onClick={() => { const newStops = [...stops]; newStops.splice(index, 1); setStops(newStops); }}><X className="w-5 h-5" /></Button>
                          </div>
                      ))}
                      <div className="flex gap-2 items-center">
                          <GoogleLocationSearch placeholder="Destino final" onSelect={setDestLocation} initialValue={destLocation?.display_name} className="flex-1" />
                          {stops.length < 2 && <Button size="icon" variant="outline" className="h-14 w-14 rounded-2xl shrink-0" onClick={() => setStops([...stops, null])}><Plus className="w-5 h-5 text-slate-400" /></Button>}
                      </div>
                  </div>
                  {pickupLocation && destLocation && <Button className="w-full h-14 bg-black text-white font-black text-lg rounded-2xl shadow-xl hover:bg-zinc-800" onClick={() => { unlockAudio(); setStep('confirm'); }}>REVISAR ROTA <ArrowRight className="ml-2 w-5 h-5" /></Button>}
              </div>
          </Card>
        )}
        ...
        {step === 'confirm' && (
          <Card className="w-full max-w-md mx-auto pointer-events-auto bg-white/95 backdrop-blur-xl p-0 rounded-[32px] shadow-2xl border border-white/40 overflow-hidden animate-in zoom-in-95">
              <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
                  <div><h2 className="text-xl font-black">Sua Viagem</h2><p className="text-xs text-slate-400 font-bold uppercase tracking-wider">{routeDistance.toFixed(1)} km • {Math.round(routeDuration)} min</p></div>
              ...
              <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                  <div className="space-y-3">
                      ...
                      <div className="flex flex-col gap-3">
                        {categories.map(cat => (
                            <button key={cat.id} onClick={() => setSelectedCategoryId(cat.id)} className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all text-left ${selectedCategoryId === cat.id ? 'border-yellow-500 bg-yellow-50/30' : 'border-slate-100 hover:border-slate-200'}`}>
                                ...
                                <div className="flex-1"><p className="font-black text-slate-900">{cat.name}</p><p className="text-[10px] text-slate-500 font-bold uppercase">Categoria Gold</p></div>
                                <div className="text-right"><p className="font-black text-lg text-slate-900">R$ {calculatePrice(cat.id).toFixed(2)}</p></div>
                            </button>
                        ))}
                      </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Pagamento</Label><div className="flex gap-2">
                          <button onClick={() => setPaymentMethod('CASH')} className={`flex-1 h-14 rounded-2xl border-2 flex items-center justify-center gap-2 transition-all ${paymentMethod === 'CASH' ? 'border-black bg-black text-white' : 'border-slate-100 text-slate-400'}`}><Banknote className="w-4 h-4" /><span className="text-xs font-black">DINHEIRO</span></button>
                      ...
                      <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Cupom</Label><div className="flex gap-2 h-14">
                          ...
                          <Button variant="outline" className="rounded-2xl h-full" onClick={applyCoupon} disabled={applyingCoupon}><Ticket className="w-4 h-4" /></Button>
                      </div></div>
                  </div>
                  <Button className="w-full h-16 bg-slate-900 text-white font-black text-xl rounded-[24px] shadow-2xl" onClick={confirmRide} disabled={isRequesting || calculatingRoute}>{isRequesting ? "SOLICITANDO..." : "SOLICITAR AGORA"}</Button>
              </div>
          </Card>
        )}
        ...
        {step === 'active' && ride && (
          <Card className="w-full max-w-md mx-auto pointer-events-auto bg-white/95 backdrop-blur-xl p-0 rounded-[32px] shadow-2xl border border-white/40 overflow-hidden animate-in slide-in-from-bottom-8">
              ...
              <div className="p-6"><div className="flex items-center gap-5 mb-6"><Avatar className="w-20 h-20 border-4 border-white shadow-xl"><AvatarImage src={ride.driver_details?.avatar_url} /><AvatarFallback className="bg-slate-200 text-slate-500 font-bold">{ride.driver_details?.first_name?.[0]}</AvatarFallback></Avatar><div className="flex-1"><h3 className="text-2xl font-black text-slate-900 leading-tight">{ride.driver_details?.first_name}</h3><Badge variant="outline" className="mt-1 border-slate-200 text-slate-500 font-bold">{ride.driver_details?.car_model} • {ride.driver_details?.car_plate}</Badge></div><div className="flex flex-col gap-3"><Button size="icon" variant="outline" className="h-12 w-12 rounded-2xl" onClick={() => setShowChat(true)}><MessageCircle className="w-5 h-5" /></Button><Button size="icon" variant="outline" className="h-12 w-12 rounded-2xl shadow-sm" onClick={() => window.open(`tel:${ride.driver_details?.phone}`)}><Phone className="w-5 h-5" /></Button></div></div><div className="flex items-center justify-between pt-2 border-t border-slate-100"><div><p className="text-[10px] font-black text-gray-400 uppercase">Pagamento</p><p className="font-black text-slate-900">{ride.payment_method}</p></div><div className="text-right"><p className="text-[10px] font-black text-gray-400 uppercase">Total</p><p className="text-2xl font-black text-slate-900">R$ {Number(ride.price).toFixed(2)}</p></div></div></div>
          </Card>
        )}
        ...
        {step === 'rating' && ride && (
          <Card className="w-full max-w-md mx-auto pointer-events-auto bg-white/95 backdrop-blur-xl p-8 rounded-[32px] shadow-2xl border border-white/40 text-center animate-in zoom-in-95">
              ...
              <div className="bg-slate-50 p-6 rounded-[32px] mb-8 mt-6"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Avalie o Motorista</p><div className="flex justify-center gap-2">{[1,2,3,4,5].map(star => (<button key={star} onClick={() => rateRide(ride.id, star, false)} className="transition-transform active:scale-90"><Star className={`w-10 h-10 ${rating >= star ? 'fill-yellow-500 text-yellow-500' : 'text-gray-200'}`} /></button>))}</div></div>
      ...
      <Dialog open={showBalanceAlert} onOpenChange={setShowBalanceAlert}>
          <DialogContent className="max-w-sm rounded-[32px] border-0 shadow-2xl p-8"><div className="text-center"><div className="w-20 h-20 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6"><Wallet className="w-10 h-10 text-red-600" /></div><DialogTitle className="text-2xl font-black text-slate-900 mb-2">Saldo Insuficiente</DialogTitle><DialogDescription className="font-medium text-slate-500 mb-6">Faltam R$ {missingAmount.toFixed(2)}. Deseja recarregar?</DialogDescription><div className="space-y-3"><Button className="w-full h-14 bg-black text-white font-black rounded-2xl" onClick={() => navigate('/wallet')}>RECARREGAR</Button><Button variant="ghost" className="w-full h-14 text-slate-500 font-bold" onClick={() => { setPaymentMethod('CASH'); setShowBalanceAlert(false); }}>USAR DINHEIRO</Button></div></div></DialogContent>
      </Dialog>
      ...
      {showChat && ride && currentUserId && (
          <RideChat rideId={ride.id} currentUserId={currentUserId} role="client" otherUserName={ride.driver_details?.first_name || 'Motorista'} otherUserAvatar={ride.driver_details?.avatar_url} onClose={() => setShowChat(false)} />
      )}
    </div>
  );
};
