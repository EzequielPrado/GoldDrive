    ...
    <div className="h-screen w-full overflow-hidden bg-gray-100 font-sans">
                    ...
                    <div className="bg-white/95 backdrop-blur-xl p-6 rounded-[32px] shadow-2xl border border-white/40 flex flex-col max-h-[80vh]">
                        ...
                        <div className="bg-gray-50 rounded-2xl p-4 mb-6 border border-gray-100 space-y-3">
                            <div className="flex items-start gap-3">
                                <div className="w-2 h-2 bg-slate-900 rounded-full mt-1.5 ring-4 ring-slate-100" />
                                <p className="text-sm font-medium text-slate-600 line-clamp-1">{pickupLocation?.display_name}</p>
                            </div>
                            <div className="flex items-start gap-3">
                                <div className="w-2 h-2 bg-yellow-500 rounded-full mt-1.5 ring-4 ring-yellow-100" />
                                    ...
                                    {categories.map(cat => {
                                        ...
                                        return (
                                            <div 
                                                ...
                                                className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all cursor-pointer ${isSelected ? 'border-yellow-500 bg-yellow-50 shadow-md' : 'border-gray-100 bg-white hover:border-gray-200'}`}
                                            >
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isSelected ? 'bg-yellow-500 text-black' : 'bg-gray-100 text-slate-400'}`}>
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-6">
                                    {appSettings.enableWallet && (
                                        <button 
                                            ...
                                            className={`h-14 rounded-2xl border-2 flex items-center justify-center gap-2 font-bold transition-all ${paymentMethod === 'WALLET' ? 'border-slate-900 bg-slate-900 text-white' : 'border-gray-200 text-slate-500'}`}
                                        >
                                    ...
                                    <button 
                                        ...
                                        className={`h-14 rounded-2xl border-2 flex items-center justify-center gap-2 font-bold transition-all ${paymentMethod === 'CASH' ? 'border-slate-900 bg-slate-900 text-white' : 'border-gray-200 text-slate-500'}`}
                                    >
                         ...
                         {ride?.status === 'SEARCHING' ? (
                             <div className="py-8 text-center">
                                 <div className="relative w-24 h-24 mx-auto mb-6">
                                    ...
                                    <div className="relative w-24 h-24 bg-yellow-500 rounded-full flex items-center justify-center text-black shadow-lg">
                             ...
                             <div className="space-y-6">
                                 <div className="flex items-center justify-between">
                                     <Badge className="bg-green-500 text-black hover:bg-green-400 font-black uppercase tracking-widest text-[10px] px-3 py-1">
                                         {ride?.status === 'ACCEPTED' ? 'MOTORISTA A CAMINHO' : ride?.status === 'ARRIVED' ? 'MOTORISTA NO LOCAL' : 'VIAGEM EM CURSO'}
                                     </Badge>
                                 ...
                                 <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                    <Avatar className="w-14 h-14 border-2 border-white shadow-sm">
                                        ...
                                        <AvatarFallback className="bg-slate-200 text-slate-600 font-bold">{ride?.driver_details?.first_name?.[0]}</AvatarFallback>
                                    </Avatar>
                                    <div className="flex-1">
                                        <h3 className="font-black text-slate-900 leading-tight">{ride?.driver_details?.first_name}</h3>
                                        <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mt-0.5">{ride?.driver_details?.car_model} • {ride?.driver_details?.car_plate}</p>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <Button size="icon" className="bg-slate-900 text-white rounded-xl h-12 w-12" onClick={() => setShowChat(true)}>
                                 ...
                                 {ride?.status !== 'IN_PROGRESS' && (
                                    <Button variant="ghost" className="w-full text-red-500 font-bold text-xs" onClick={() => setShowCancelAlert(true)}>Cancelar Viagem</Button>
                                 )}
                             </div>
                         )}
                     </div>
                        ...
                        <div className="flex justify-center gap-3 mb-8">
                            {[1,2,3,4,5].map(star => (
                                <button key={star} onClick={() => setRating(star)} className="transition-transform active:scale-90">
                                    <Star className={`w-10 h-10 ${rating >= star ? 'fill-yellow-500 text-yellow-500' : 'text-gray-200'}`} />
                                </button>
                            ))}
                        </div>

                        <Button className="w-full h-14 bg-black text-white font-bold rounded-2xl text-lg shadow-xl" onClick={async () => {
                            if (ride) await rateRide(ride.id, rating || 5, false);
                            clearRide();
                            setStep('search');
                        }}>
                            Finalizar
                ...
                <ScrollArea className="flex-1 -mr-2 pr-4 custom-scrollbar">
                    {historyItems.length === 0 ? (
                        <div className="py-12 text-center text-gray-400">
                            ...
                            <p>Você ainda não realizou viagens.</p>
                        </div>
                    ) : historyItems.map(item => (
                        <div key={item.id} className="mb-3 p-4 bg-gray-50 hover:bg-white border border-gray-100 rounded-2xl transition-all cursor-pointer group shadow-sm">
                            <div className="flex justify-between items-start mb-2">
                                <div>
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{new Date(item.created_at).toLocaleDateString()}</p>
                                    <p className="font-black text-slate-900 text-sm mt-0.5">{item.category}</p>
                                </div>
                                <span className="font-black text-slate-900">R$ {Number(item.price).toFixed(2)}</span>
                            </div>
                            <div className="flex items-center gap-2 mt-3 text-xs text-gray-500">
                                <MapPin className="w-3 h-3 shrink-0" />
                                <p className="truncate">{item.destination_address}</p>
                            </div>
                        </div>
                    ))}
                </ScrollArea>
      ...
      <AlertDialog open={showCancelAlert} onOpenChange={setShowCancelAlert}>
          <AlertDialogContent className="rounded-3xl border-0 shadow-2xl">
              <AlertDialogHeader>
                  <AlertDialogTitle className="text-2xl font-black text-slate-900">Cancelar Viagem?</AlertDialogTitle>
                  <AlertDialogDescription className="text-gray-500">
                      O cancelamento frequente pode gerar taxas na sua próxima corrida. Tem certeza?
                  </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter className="mt-4 flex gap-3">
                  <AlertDialogCancel className="rounded-xl h-12 flex-1 font-bold">Voltar</AlertDialogCancel>
                  <AlertDialogAction className="bg-red-600 hover:bg-red-700 text-white rounded-xl h-12 flex-1 font-bold" onClick={async () => {
                      if(ride) await cancelRide(ride.id, "Cancelado pelo passageiro");
      ...
      <Dialog open={showBalanceAlert} onOpenChange={setShowBalanceAlert}>
          <DialogContent className="rounded-[32px] border-0 text-center p-8">
              ...
              <DialogDescription className="text-gray-500 mb-6">
                  Você precisa de mais <span className="font-black text-red-600">R$ {missingAmount.toFixed(2)}</span> para realizar esta viagem usando a carteira.
              </DialogDescription>
