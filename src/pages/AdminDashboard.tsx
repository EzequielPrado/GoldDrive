                                      ...
                                      <div className="flex flex-col md:flex-row gap-6 items-start md:items-end">
                                          <div className="flex-1 w-full space-y-1 text-left">
                                              <div className="flex items-center justify-between mb-2">
                                                  <div className="flex items-center gap-2 flex-1"><Car className={`w-5 h-5 shrink-0 ${cat.active ? 'text-yellow-500' : 'text-slate-300'}`} /><div className="relative flex-1 max-w-[300px] group"><Input value={cat.name} onChange={e => handleCategoryChange(cat.id, 'name', e.target.value)} className="font-black text-slate-900 text-lg h-10 border-0 bg-transparent focus-visible:ring-1 focus-visible:ring-yellow-500/50 px-0 hover:bg-slate-100/50 transition-all rounded-lg" placeholder="Nome da Categoria" /><Pencil className="w-3 h-3 absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" /></div></div>
