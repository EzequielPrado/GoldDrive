...
const AdminDashboard = () => {
  ...
  const fetchData = async (isManual = false) => {
    ...
    try {
        ...
        // Construindo a query de rides dinamicamente
        let ridesQuery = supabase.from('rides').select(`*, driver:profiles!public_rides_driver_id_fkey(*), customer:profiles!public_rides_customer_id_fkey(*)`);

        if (filterStatus === 'ALL') {
            // Filtra rides 'SEARCHING' que são mais antigas que 10 minutos
            const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000).toISOString();
            ridesQuery = ridesQuery.or(`status.neq.SEARCHING,and(status.eq.SEARCHING,created_at.gte.${tenMinutesAgo})`);
        } else {
        ...
        const [ridesRes, profilesRes, settingsRes, pricingRes, catRes, adminConfigRes] = await Promise.all([
            ridesQuery.order('created_at', { ascending: false }),
